...
name: investment
version: "0.2.0"
description: "개인 투자 의사결정 메타 라우터 — 포트폴리오 이론·시스템 규율·행동 편향·한국 세제·FIRE 12개 프레임워크에서 상황에 맞는 것을 자동 선택. 자산 배분·하락장 방어·세금 최적화·은퇴 계획까지 커버. 종목 추천 금지, 2026.4 기준."
tools: ["Read", "Skill"]
subskills:
...
- **`real-estate`** — 부동산 자산 분석(hedonic·재건축·갭투자·청약)은 별도 레포. 이 레포에선 *포트폴리오 내 부동산 비중·리밸런싱 트리거·주거 vs 투자용 판단*만 다룸. 주택 개별 매매 질문은 `real-estate`로.
- **`learning-framework`** — 재테크 공부 설계(6개월+ 깊이)는 `learning-framework` + 이 레포를 MOC로. 특히 `behavioral-biases` 극복은 *지속 학습 + 외부 책무* 필요.
- **`health-framework`** — 장수·건강보험료·의료비는 장기 재무의 외생 변수. FIRE·은퇴 계획 시 `health-framework`의 `blood-biomarkers`·`sleep-foundations` 결과를 시나리오에 통합.
...
- **모른다고 말할 것** — 라우터가 답할 수 없는 영역(개별 종목 분석·시장 전망·사업자 세무·상속 설계)은 명시적으로 범위 밖을 선언하고 `real-estate`·전문가 상담으로 안내.
